  ...
  var QualityManager = class {
    ...
    setPlayerQuality(targetQuality) {
      ...
      if (targetQuality.toLowerCase() === "auto") {
        const autoQuality = qualityList.find(
          (q) => q.label.toLowerCase().includes("auto")
        );
        if (autoQuality) {
          ...
          if (this.debug)
            console.log(
              "[QualityManager] Qualit\xE9 d\xE9finie sur automatique :",
              finalQuality
            );
          return;
        }
        ...
        if (exactMatch && !exactMatch.isPremium) {
          ...
          const lowerQualities = qualityList.filter(
            (q) => q.resolution !== null && q.resolution <= targetResolution && !q.isPremium
